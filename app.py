...
class CloneOnlyApp:
    def __init__(self) -> None:
        ...
        self.status_var = tk.StringVar(value="Load a settings JSON file, then click `Clone Repository`.")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header,
            text="A simple app that takes one GitHub repository URL and clones it locally.",
            bg="#173a2d",
            fg="#d7e5dc",
            font=("Helvetica", 12),
            justify="left",
        ...
        tk.Label(info, text="Last Configured URL", bg="#eef3ef", fg="#334155", font=("Helvetica", 10, "bold")).pack(anchor="w")
        tk.Label(info, text=self.repo_url or "(none)", bg="#eef3ef", fg="#0f172a", font=("Menlo", 10), justify="left", wraplength=940).pack(anchor="w", pady=(4, 8))
        tk.Label(info, text="Local Clone Path", bg="#eef3ef", fg="#334155", font=("Helvetica", 10, "bold")).pack(anchor="w")
    ...
    def create_template_copy(self) -> None:
        ...
        self.status_var.set(f"Settings template created: {destination}")
    ...
    def load_settings_file(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Failed to Read Settings", str(exc))
            return

        repo_url = str(settings.get("repo_url", "")).strip()
        if not repo_url:
            messagebox.showerror("Settings Error", "`repo_url` is empty.")
            return

        self.repo_url = repo_url
        self.config["repo_url"] = repo_url
        self.config_store.save(self.config)
        self.status_var.set(f"Settings loaded. Click `Clone Repository` next.\nURL: {repo_url}")
        self._log(f"Loaded settings: {file_path}")

    def clone_repo(self) -> None:
        if not self.repo_url:
            messagebox.showwarning("URL Required", "Load a settings JSON file first.")
            return
        ...
        if destination.exists():
            messagebox.showwarning("Already Exists", f"A folder with the same name already exists.\n{destination}")
        ...
        if result.returncode != 0:
            ...
            messagebox.showerror("Clone Failed", result.stderr.strip() or "git clone failed")
            return
        ...
        self.status_var.set(f"Repository cloned locally.\n{destination}")
        messagebox.showinfo("Done", f"Repository cloned.\n{destination}")
